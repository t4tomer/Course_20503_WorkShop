...
public class IndexController {
	...
	// $------> the methods below are used for the option for the user to change his
	// settings
	...
	public String validateForChangingUserSettings(@RequestParam("email") String email, @RequestParam String authCode,
		...
		if (inputEmail.equals(validateEmail) && inputPassword.equals(validatePassWord)) {
			...
			//// ! send mail in order to change user settings
			try {
				senderService.sendSimpleEmail(inputEmail, "Code for changing settings", NewTempPswd);
			} catch (MessagingException e) {
